...
export class FoodComponent implements OnInit {
  ...
  saveFood() {
    if (this.currentFoodItem.id) {
      ...
      this._dataService.addFood(this.currentFoodItem.itemName).subscribe(
        data => {
          this.currentFoodItem = new FoodItem();
        },
        error => {
  ...
  deleteFoodItem(foodItem: FoodItem) {
    this._dataService.deleteFood(foodItem.id).subscribe(
      response => {
  ...
  private getAllFood(): void {
    this._ngZone.run(() => {
      this._dataService.getAllFood().subscribe(
        data => {
          this.foodItems = data;
        },
        ...
        () =>
          console.log(
            'Get all Foods complete ' + JSON.stringify(this.foodItems)
          )
      );
    });
  }
